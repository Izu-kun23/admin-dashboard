...
import { Activity, Clock, User, CheckCircle, XCircle, Search, Calendar, MoreHorizontal, Plus, Table as TableIcon, Filter, ArrowUpDown, Users, DollarSign, Tag, Calendar as CalendarIcon, Trash2 } from 'lucide-react'
...
export default function ActivityPage() {
  ...
  const fetchQuizSubmissions = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log('✅ Quiz submissions loaded:', result.count || 0, 'records')
      } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/quiz-submissions/${submissionToDelete.id}`, {
        method: 'DELETE',
      })
  ...
  const inProgressSubmissions = submissions.filter(s => s.logo_status === 'in_progress')
  const completedSubmissions = submissions.filter(s => s.logo_status === 'completed')
    ...
    if (filteredSubmissions.length === 0) {
      return (
        <div className="text-center py-10">
          ...
          <h3 className="text-sm font-medium text-gray-700 mb-1">No Submissions Found</h3>
              ...
              <TableRow key={submission.id} className="hover:bg-gray-50 border-b border-gray-100">
                ...
                <TableCell>
                  <div className="flex flex-wrap gap-0.5 max-w-[200px]">
                    {submission.brand_goals && submission.brand_goals.length > 0 ? (
                      <>
                        {submission.brand_goals.slice(0, 3).map((goal, goalIndex) => (
                          <Badge 
                            key={goalIndex} 
                            variant="outline" 
          ...
          <div className="space-y-6">
            ...
            <Tabs defaultValue="brand-submissions" className="w-full">
              <TabsList className="grid w-full grid-cols-3 bg-gray-50 h-9">
                <TabsTrigger value="brand-submissions" className="text-xs data-[state=active]:bg-white data-[state=active]:text-gray-900">
                  Brand Submissions
                  <Badge variant="secondary" className="ml-1.5 text-[10px] px-1.5 py-0">
                    {allSubmissions.length}
                  </Badge>
                </TabsTrigger>
                <TabsTrigger value="in-progress" className="text-xs data-[state=active]:bg-white data-[state=active]:text-gray-900">
                  In Progress
                  <Badge variant="secondary" className="ml-1.5 text-[10px] px-1.5 py-0">
                    {inProgressSubmissions.length}
                  </Badge>
                </TabsTrigger>
                <TabsTrigger value="completed" className="text-xs data-[state=active]:bg-white data-[state=active]:text-gray-900">
                  Completed
                  <Badge variant="secondary" className="ml-1.5 text-[10px] px-1.5 py-0">
        ...
        <DialogContent className="sm:max-w-md bg-white border border-gray-200 rounded-lg">
          <DialogHeader className="text-center space-y-4">
            ...
            <DialogDescription className="text-gray-600 text-sm leading-relaxed">
              ...
              <span className="font-semibold text-gray-900">{submissionToDelete?.brand_name}</span>{' '}
              by <span className="font-semibold text-gray-900">{submissionToDelete?.full_name}</span>{' '}
